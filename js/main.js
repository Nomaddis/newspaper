...
get('../db.json').then(function(response) {
    let data = JSON.parse(response);
    buildPostsFromData(data);
}, function(error) {
...
/**
 * create posts in DOM
 * @param posts posts from data
 */
function buildPostsFromData(posts) {
    let newPosts = '';
    for(let i = 0; i<posts.length;  i++) {

        if( i!==(posts.length-1) ) {
            newPosts += cretePost(posts[i].date, posts[i].title, posts[i].description, posts[i].img);
        } else {
            newPosts += cretePost(posts[i].date, posts[i].title, posts[i].description, posts[i].img, 'news__item_last');
        }
    }
    news.innerHTML = newPosts;
}

/**
 * function build string DOM post
 * @param day - day of post
 * @param title - title of post
 * @param desc - description of post
 * @param img - image of post
 * @param indicator - indicate last post
 * @returns {string} - string ofDOM element
 */
function cretePost(day, title, desc, img, indicator = '') {
    let post="";
    post += `<div class=\"news__item ${indicator}\">`;
    post += "          <div class=\"news_item__photo\">";
    post += `            <div class=\"img-cont\"><img src=\"img\/${img}\"><\/div>`;
    post += "          <\/div>";
    post += "          <div class=\"news_item__content-wrap\">";
    post += "            <div class=\"news_item__content\">";
    post += "              <div class=\"content__title\">";
    post += `                <h2>${title}<\/h2>`;
    post += `                <div class=\"news_date\"><span>${day}<\/span><\/div>`;
    post += "              <\/div>";
    post += "              <div class=\"content__text\">";
    post += `                <p>${desc}<\/p>`;
    post += "              <\/div>";
    post += "              <div class=\"content__img\">";
    post += `                <div class=\"img-cont\"><img src=\"img\/${img}\" alt=\"video\"><\/div>`;
    post += "                <div class=\"play_sircle\"><\/div>";
    post += "              <\/div>";
    post += "            <\/div>";
    post += "          <\/div>";
    post += "        <\/div>";
